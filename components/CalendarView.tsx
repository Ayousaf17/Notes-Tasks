...
export const CalendarView: React.FC<CalendarViewProps> = ({ tasks, onSelectTask }) => {
  ...
  const getPriorityColor = (p?: TaskPriority) => {
    switch(p) {
        ...
        case TaskPriority.MEDIUM: return 'bg-orange-100 text-orange-700 border-orange-200';
    ...
    <div className="flex-1 h-full flex flex-col bg-white overflow-hidden">
      ...
      <div className="flex-1 grid grid-cols-7 auto-rows-fr bg-gray-50/30 overflow-y-auto">
        ...
        {Array.from({ length: firstDay }).map((_, i) => (
            <div key={`empty-${i}`} className="border-b border-r border-gray-100 bg-gray-50/50 min-h-[120px]"></div>
        ))}
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
                <div key={day} className={`group border-b border-r border-gray-100 p-2 min-h-[120px] transition-colors hover:bg-white relative flex flex-col gap-1 ${isToday ? 'bg-white ring-1 ring-inset ring-blue-100' : ''}`}>
                    ...
                    <div className="flex-1 flex flex-col gap-1 overflow-y-auto max-h-[120px] no-scrollbar">
                        {dayTasks.map(task => (
                            <button 
                                ...
                                className={`text-left px-2 py-1.5 rounded text-[10px] font-medium border truncate w-full transition-transform hover:scale-[1.02] ${getPriorityColor(task.priority)} ${task.status === TaskStatus.DONE ? 'opacity-50 line-through grayscale' : ''}`}
                                title={task.title}
                            >
                                {task.title}
                            </button>
