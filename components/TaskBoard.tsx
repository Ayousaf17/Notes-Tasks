...
export const TaskBoard: React.FC<TaskBoardProps> = ({ 
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background font-sans overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-background/80 backdrop-blur z-20">
         <div className="flex gap-4"><select onChange={(e)=>setAssigneeFilter(e.target.value)} className="bg-transparent text-sm text-foreground"><option value="ALL">All</option>{availableAssignees.map(u=><option key={u} value={u}>{u}</option>)}</select></div>
         <button onClick={handleSuggestTasks} disabled={isSuggesting} className="text-sm text-primary flex gap-2 items-center">{isSuggesting ? <Loader2 className="w-4 h-4 animate-spin"/> : <Sparkles className="w-4 h-4"/>} Suggest</button>
      </div>
        ...
        <div className="flex h-full gap-6">
            {columns.map(col => (
                <div key={col.id} className="flex-1 min-w-[300px] flex flex-col" onDragOver={(e) => handleDragOver(e, col.id)} onDrop={(e) => handleDrop(e, col.id)}>
                    <h3 className="font-bold text-sm text-muted-foreground mb-3">{col.label}</h3>
                    <div className="flex-1 overflow-y-auto space-y-3">
                        {filteredAndSortedTasks.filter(t => t.status === col.id).map(task => (
                            <div key={task.id} draggable onDragStart={(e) => handleDragStart(e, task.id)} onClick={() => onSelectTask && onSelectTask(task.id)} className="bg-card border border-border p-4 rounded-lg shadow-sm hover:shadow-md cursor-pointer group relative transition-all">
                                {task.relatedClientId && (
                                    <div className="absolute top-3 right-3 text-[10px] text-accent-foreground font-bold flex items-center gap-1 bg-accent px-1.5 py-0.5 rounded">
                                        <Briefcase className="w-3 h-3" /> Client
                                    </div>
                                )}
                                <div className="text-sm font-medium text-card-foreground mb-1 pr-6 leading-snug">{task.title}</div>
                                ...
                                <div className="flex justify-between items-center mt-3 pt-3 border-t border-border">
                                    <div className="flex items-center gap-2">
                                        {task.assignee && (
                                            <div className="w-6 h-6 rounded-full bg-secondary border border-border flex items-center justify-center text-[10px] text-secondary-foreground font-medium">
                                                {task.assignee.charAt(0)}
                                            </div>
                                        )}
                                        {task.dueDate && (
                                            <div className="text-[10px] text-muted-foreground flex items-center gap-1.5 bg-muted px-2 py-1 rounded-md border border-border">
                                    ...
                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            ...
                                            className="p-1.5 hover:bg-accent rounded text-muted-foreground hover:text-accent-foreground transition-colors"
                                            title="Discuss with AI"
                                        >
                                            <MessageSquare className="w-3.5 h-3.5" />
                                        </button>
                                        <button onClick={(e) => { e.stopPropagation(); onDeleteTask(task.id); }} className="p-1.5 hover:bg-destructive/10 rounded text-muted-foreground hover:text-destructive transition-colors">
                        ...
                        <button onClick={() => onAddTasks([{ title: '', status: col.id as TaskStatus }])} className="w-full py-2 border border-dashed border-border rounded text-muted-foreground text-xs hover:border-foreground/30 hover:text-foreground transition-colors">+ New Task</button>
                    </div>
                </div>
            ))}
        </div>
